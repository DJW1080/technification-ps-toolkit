...
$script:ModuleName = 'network-diagnostics'
$script:SessionLog = New-TechnificationLogFile -ModuleName $script:ModuleName -Prefix 'session'
Write-TechnificationLog -Path $script:SessionLog -Level 'info' -Message 'Network Diagnostics session started.'
...
function Show-ActiveConnections {
    ...
    try {
        ...
        if (-not $connections -or $connections.Count -eq 0) {
            Write-Warn 'No active TCP connections were found.'
            return
        }
...
function Export-NetworkReport {
    $reportPath = New-TechnificationReportFile -ModuleName $script:ModuleName -Prefix 'network-report' -Extension 'txt'
    ...
    Write-TechnificationLog -Path $script:SessionLog -Level 'info' -Message ('Network report saved: {0}' -f $reportPath)
}
...
function Show-NetworkDiagnosticsPage {
    $header = New-MenuHeader -Name 'Network Diagnostics' -Version '1.2' -InfoLines @(
        ("Logs    : {0}" -f (Get-TechnificationLogsPath)),
        ("Reports : {0}" -f (Get-TechnificationReportsPath))
    )
    Show-MenuPage -Title 'Diagnostics Menu' -Items (& $script:GetNetworkItems) -HeaderLines $header
}
...
Write-TechnificationLog -Path $script:SessionLog -Level 'info' -Message 'Network Diagnostics session ended.'
